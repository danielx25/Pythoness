...
namespace RedNeuronal
{
	...
	double BP::deltaPeso(int idc_nrn, int idc_nrn_anterior)
	{
		deltas[idc_nrn] = tasa_aprdj * salidas[idc_nrn_anterior] * gradiantes[idc_nrn];

		return deltas[idc_nrn];
	}

	void BP::setErrores(double* salida_deseada)
	{
	...
	void BP::ajustarPeso(int idc_nrn, int idc_capa)
	{
		...
		for (int j = idc_pr_nrn; j < idc_ult_nrn; j++)
			pesos[j][idc_nrn] += deltaPeso(idc_nrn, j);
	}

	void BP::aprendizaje()
	{
		for (int capa = 1; capa <= (num_capas - 1); capa++)
		{
			int idc_pr_nrn = getPrimeraNeurona(capa);
			int idc_ult_nrn = getUltimaNeurona(capa);
